<template>
        ...
        <v-row class="px-5">
            <section>
                <span class="subtitle-2 font-weight-bold">Completed <span class="caption">({{completed_appointments.length}} appointments)</span></span>
            </section>
            <v-spacer />
            <v-icon color="info darken-1">mdi-account-group</v-icon>
        </v-row>
            <v-row v-if="completed_appointments.length">
                <v-col cols="6" md="3" sm="4" v-for="appointment in completed_appointments" :key="appointment.id">
                    <v-card outlined>
...
export default {
    ...
    computed:{
        ...mapGetters(['completed_appointments']),
    },
    watch:{
    ...
    methods:{
        ...
        appontment_details(appointment){
            this.$router.push({name: 'appointment_completed_details', params: {id: appointment.id}});
        },
