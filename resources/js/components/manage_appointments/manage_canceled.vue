<template>
    <v-container>
        ...
        <v-row class="px-5">
            <section>
                <span class="subtitle-2 font-weight-bold">Canceled <span class="caption">({{manage_canceled_appointments.length}} appointments)</span></span>
            </section>
            <v-spacer />
            <v-icon color="info darken-1">mdi-account-group</v-icon>
        </v-row>
            <v-row v-if="manage_canceled_appointments.length">
                <v-col cols="6" md="3" sm="4" v-for="appointment in manage_canceled_appointments" :key="appointment.id">
                    <v-card outlined>
...
export default {
    ...
    computed:{
        ...mapGetters(['manage_canceled_appointments']),
    },
    mounted () {
    ...
    methods:{
        ...
        appontment_details(appointment){
            this.$router.push({name: 'manage_appointment_canceled_details', params: {id: appointment.id}});
        },
            ...
            swalWithBootstrapButtons.fire({
                ...
                if (result.value) {
                    ...
                    if(response.data.success){
                        ...
                        var index = this.manage_canceled_appointments.indexOf(appointment);
