<template>
    <v-container>
        ...
        <v-row class="px-5">
            <section>
                <span class="subtitle-2 font-weight-bold">Today <span class="caption">({{manage_upcoming_appointments.length}} appointments)</span></span>
            </section>
            <v-spacer />
            <v-icon color="info darken-1">mdi-account-group</v-icon>
        </v-row>
            <v-row v-if="manage_upcoming_appointments.length">
                <v-col cols="6" md="3" sm="4" v-for="appointment in manage_upcoming_appointments" :key="appointment.id">
                    <v-card outlined>
...
export default {
    ...
    methods:{
        ...mapActions(['fetch_all_today', 'fetch_all_canceled', 'fetch_all_upcoming']),

        appontment_details(appointment){
            this.$router.push({name: 'manage_appointment_upcoming_details', params: {id: appointment.id}});
        },
