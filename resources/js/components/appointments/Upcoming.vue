<template>
        ...
        <v-row class="px-5">
            <section>
                <span class="subtitle-2 font-weight-bold">Upcoming <span class="caption">({{upcoming_appointments.length}} appointments)</span></span>
            </section>
            <v-spacer />
            <v-icon color="info darken-1">mdi-account-group</v-icon>
        </v-row>
            <v-row v-if="upcoming_appointments.length">
                <v-col cols="6" md="3" sm="4" v-for="appointment in upcoming_appointments" :key="appointment.id">
                    <v-card outlined>
...
export default {
    ...
    methods:{
        ...
        appontment_details(appointment){
            this.$router.push({name: 'appointment_upcoming_details', params: {id: appointment.id}});
        },
            ...
            swalWithBootstrapButtons.fire({
                ...
                if (result.value) {
                    ...
                    if(response.data.success){
                        ...
                        var index = this.upcoming_appointments.indexOf(appointment);
