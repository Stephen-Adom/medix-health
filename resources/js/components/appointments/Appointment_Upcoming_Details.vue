                                ...
                                <td class="text-center"> 
                                    <v-tooltip top>
                                        <template v-slot:activator="{on}">
                                            <v-btn class="ma-2" v-on="on" fab dark color="success" x-small @click="accept_appointment">
...
export default {
    ...
    computed:{
        ...mapGetters(['upcoming_appointments']),

        appointment(){
            return this.upcoming_appointments.find(appointment => appointment.id == this.id);
        },
        getImage(){
            if(this.appointment){
    ...
    methods:{
        ...mapActions(['fetchUpcomingAppointment', 'fetchCanceledAppointment']),
            ...
            swalWithBootstrapButtons.fire({
                ...
                if (result.value) {
                    ...
                    if(response.data.success){
                        ...
                        var index = this.upcoming_appointments.indexOf(this.appointment);
                        this.upcoming_appointments.splice(index, 1);
                        swalWithBootstrapButtons.fire(
                            'Canceled!',
        ...
        accept_appointment(){
            ...
            swalWithBootstrapButtons.fire({
                ...
                if (result.value) {
                    this.loading_dialog = true;
                    axios.patch('/api/accept_appointment/' + this.appointment.id).then(response => {
                    if(response.data.success){
                        this.goback();
                        this.fetchUpcomingAppointment();
                        this.$refs.topProgress.done();
        ...
        newBooked(){
            if(this.appointment_time === null && this.appointment_time === null){
                ...
                this.$refs.topProgress.start();
                this.loading_dialog = true;
                axios.patch('/api/reschedule_appointment/'+ this.appointment.patient.id, {appointment_time: this.appointment_time, appointment_day: this.appointment_day}).then(response => {
                    if(response.data.success){
                        this.goback();
                        this.$refs.topProgress.done();
                        this.loading_dialog = false;
