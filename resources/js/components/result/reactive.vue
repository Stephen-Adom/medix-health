<template>
    <div>
        <v-container class="mt-5 ma-3">
              <v-card class="pa-4" outlined v-if="allReactive">
                <v-data-table :headers="headers" :search="search" :items="allReactive" :loading ="loadingData" loading-text="Loading... Please wait">
                    <template v-slot:top>
                    <v-toolbar flat color="white">
                        <v-toolbar-title>C-Reactive Protein Test Result</v-toolbar-title>
                    ...
                    <template v-slot:item.firstname="{item}">
                        <span v-if="allReactive" class="caption font-weight-bold">{{item.patient.firstname}} {{item.patient.lastname}}</span>
                    </template>
                    <template v-slot:item.gender="{item}">
                        <span v-if="allReactive" class="caption font-weight-bold">{{item.patient.gender}}</span>
                    </template>
                    <template v-slot:item.dob="{item}">
                        <span v-if="allReactive" class="caption font-weight-bold">{{item.patient.dob | formatDob}}</span>
                    </template>
                    <template v-slot:item.examined="{item}">
                        <span v-if="allReactive" class="caption font-weight-bold">{{item.user.title}} {{item.user.lastname}}, {{item.user.firstname}}</span>
                    </template>
                    <template v-slot:item.created_at="{item}">
                        <span v-if="allReactive" class="caption font-weight-bold">{{item.created_at | formatDate}}</span>
                    </template>
                                    ...
                                    <v-container>
                                        <v-card outlined>
                                            <v-card-title>
                                                <v-row>
                                                    ...
                                                    <v-col cols="6" class="mt-10 text-center">
                                                        ...
                                                        <h5 class="subtitle-2 font-weight-bold text-uppercase">Sulphate Serum Test</h5>
                                            ...
                                            <v-card outlined class="mt-5">
                                                <v-card-title class="subtitle-2">
                                                    <span class="subtitle-2 font-weight-bold text-uppercase">C-Reactive Protein Test</span>
                                                </v-card-title>
                                                <v-divider />
                                                <v-card-text>
                                                    <v-simple-table>
                                                        ...
                                                        <tbody>
                                                            <tr>
                                                                <td class="subtitle-2 font-weight-bold">C-Reactive Protein</td>
                                                                <td>
                                                                    <v-text-field dense class="subtitle-2 font-weight-bold" v-model="form.reactive_protein" readonly></v-text-field>
                                                                </td>
                                                                <td>
...
export default {
    ...
    computed:{
        ...mapGetters(['allReactive']),
    },
    methods:{
        ...mapActions(['fetchReactive']),
